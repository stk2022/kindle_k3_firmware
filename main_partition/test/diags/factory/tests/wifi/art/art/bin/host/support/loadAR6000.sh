...
# If the board is not Luigi or Shasta, do not attempt to load WiFi
...
if [ "$bmi_enable" != "yes" ]; then # {
    ...
    if [ "$TARGET_TYPE" = "AR6002" -o "$TARGET_TYPE" = "AR6003" ]; then
	    ...
	    if [ "$DEEP_SLEEP" -eq 0 ]; then 
            ...
            echo "DEEP_SLEEP is not set"
	    else
	        echo "DEEP_SLEEP is set"
        fi
    ...
    if [ "$RPDF_FILE" = "" ]; then
        ...
        if [ "$TARGET_TYPE" = "AR6001" ]; then
            RPDF_FILE=$ART_WORKAREA/target/AR6001/bin/patch.rpdf
        else
          if [ "$TARGET_VERSION" = "$AR6002_VERSION_REV2" ]; then
            RPDF_FILE=$ART_WORKAREA/target/AR6002/hw2.0/bin/patch.rpdf
          fi
        fi
    fi
    ...
    if [ \( "$EEPROM" = "" \) -a \( "$TARGET_TYPE" = "AR6002" \) -a \( "$macaddr" != "" \) ]
    then
    	EEPROM=$IMAGEPATH/ar6002_eeprom_file.bin
    	
    	# Transfer Board Data from Target EEPROM to a file first and then update MAC address
        $IMAGEPATH/eeprom.$TARGET_TYPE --read --file=$EEPROM --interface=$NETIF
    fi
    ...
    if [ "$TARGET_TYPE" = "AR6002" ]; then
        if [ "$EEPROM" != "" ]; then
            ...
            if [ "$macaddr" != "" ]; then
                ...
                $IMAGEPATH/eeprom.$TARGET_TYPE --transfer --file=$EEPROM --setmac=$macaddr --interface=$NETIF
            else
                $IMAGEPATH/eeprom.$TARGET_TYPE --transfer --file=$EEPROM --interface=$NETIF
            fi
            ...
            if [ "$TARGET_VERSION" = "$AR6002_VERSION_REV2" ]; then
                ...
                $IMAGEPATH/bmiloader -i $NETIF --write --address=0x513950 --file=$WORKAREA/target/AR6002/hw2.0/bin/eeprom.bin
                $IMAGEPATH/bmiloader -i $NETIF --execute --address=0x913950 --param=0
            fi
        fi
        # Set the new begin address for the PC to jump to.
        $IMAGEPATH/bmiloader -i $NETIF --begin --address=0x913950
    fi
    ...
    if [ "$TARGET_TYPE" = "AR6002" -o "$TARGET_TYPE" = "AR6003" ]; then
        ...
        if [ "$TARGET_VERSION" = "$AR6002_VERSION_REV2" ]; then
            LOADADDR=0x502070
            wlanapp=${wlanapp:-$ART_WORKAREA/target/AR6002/hw2.0/bin/athwlan.bin}
        ...
        if [ "$TARGET_TYPE" = "AR6003" ]; then
            LOADADDR=0x544000
            wlanapp=${wlanapp:-$ART_WORKAREA/target/AR6003/fpga/bin/athwlan.bin}
        fi
        ...
        # WLAN Patch DataSets
        if [ "$TARGET_VERSION" = "$AR6002_VERSION_REV2" ]; then
            $IMAGEPATH/bmiloader -i $NETIF --write --address=0x52d6d0 --file=$WORKAREA/target/AR6002/hw2.0/bin/data.patch.hw2_0.bin
            $IMAGEPATH/bmiloader -i $NETIF --write --address=0x500418 --param=0x52d6d0
        fi
    ...
    if [ \( $exit_value -eq 0 \) -a \( $START_WLAN = TRUE \) ]; then
        ...
        if [ "$DEEP_SLEEP" -eq 0 ]; then
	        ...
	        echo "DEEP_SLEEP is not enabled"
	    else
	        echo "DEEP_SLEEP is disabled"
	    fi
    fi
fi # }
